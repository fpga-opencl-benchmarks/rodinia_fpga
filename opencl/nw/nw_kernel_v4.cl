...
__kernel void
nw_kernel1(int  blk,
           __global int* RESTRICT reference_d, 
           __global int* RESTRICT input_itemsets_d, 
           __attribute__((local_mem_size(sizeof(int) * BSIZE * BSIZE * 2)))
           __local  int* RESTRICT input_itemsets_l,
           __attribute__((local_mem_size(sizeof(int) * BSIZE * BSIZE)))
           __local  int* RESTRICT reference_l,
           int  cols,
           int  penalty,
           int  block_width,
           int  offset_r,
...
{  
  ...
  int b_index_x = is_upper_left ? bx : (bx + block_width - blk);
  int b_index_y = is_upper_left ? (blk - 1 - bx) : (block_width - bx -1);
