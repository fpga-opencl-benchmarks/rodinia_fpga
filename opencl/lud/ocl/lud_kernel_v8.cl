...
__attribute__((max_work_group_size(2*BSIZE))) // should be reqd, used max instead to force the compiler into reducing the number of simultaneous work groups and over-replicating the local buffers
                                                  ...
                                                  int             offset)
{
	...
	array_offset2 = offset * matrix_dim + (bx+1) * BSIZE + offset;
	for (i=0; i < BSIZE; i++)
	{
		dia_row[i * BSIZE + tx] = m[array_offset1 + tx];
		peri_row[tx * BSIZE + i] = m[array_offset2 + tx];
	...
	array_offset1 = offset * matrix_dim + (bx+1) * BSIZE + offset;
	for(i=0; i < BSIZE; i++)
	{
		float sum = 0.0f;
		...
		for (j=0; j < i; j++)
		{
			sum += dia_row[i * BSIZE + j] * peri_row[tx * BSIZE + j];
		}
		peri_row[tx * BSIZE + i] -= sum;

		m[array_offset1 + tx] = peri_row[tx * BSIZE + i];
		array_offset1 += matrix_dim;
	}
}
                                                  ...
                                                  int             offset)
{
	...
	array_offset1 = offset * matrix_dim + offset;
	array_offset2 = (offset + (bx+1) * BSIZE) * matrix_dim + offset;
	for (i=0; i < BSIZE; i++)
	{
		dia_col[tx * BSIZE + i] = m[array_offset1 + tx];
		peri_col[i * BSIZE + tx] = m[array_offset2 + tx];
	...
	for(i=0; i < BSIZE; i++)
	{
		...
		for(j=0; j < i; j++)
		{
			sum += dia_col[i * BSIZE + j] * peri_col[tx * BSIZE + j];
		}
		peri_col[tx * BSIZE + i] = (peri_col[tx * BSIZE + i] - sum) / dia_col[i * BSIZE + i];
	}

	//barrier(CLK_LOCAL_MEM_FENCE);

	array_offset1 = (offset + (bx+1) * BSIZE) * matrix_dim + offset;
	#pragma unroll 16
	for(i=0; i < BSIZE; i++)
	{
		m[array_offset1 + tx * matrix_dim + i] = peri_col[tx * BSIZE + i];
	}
}
