                                              ...
                                              int             offset)
{
	...
	array_offset[1] = (offset + txg) * matrix_dim + (bx+1) * BSIZE + offset;
	array_offset[2] = (offset + txg + (bx+1) * BSIZE) * matrix_dim + offset;

	for (i=0; i < BSIZE; i=i+2)
	{
		dia_row[(i + txg) * BSIZE + idx]  = m[array_offset[0] + idx];
		dia_col[idx * BSIZE + (i + txg)]  = m[array_offset[0] + idx];
		peri_row[idx * BSIZE + (i + txg)] = m[array_offset[1] + idx];
		peri_col[(i + txg) * BSIZE + idx] = m[array_offset[2] + idx];
	...
	{ //peri-row
		idx = tx;
		int peri_row_array_offset = offset * matrix_dim + (bx+1) * BSIZE + offset;
		for(i=0; i < BSIZE; i++)
		{
			float sum = 0.0f;
	...
	{ //peri-col
		idx = tx - BSIZE;
		int peri_col_array_offset = (offset + (bx+1) * BSIZE) * matrix_dim + offset;
		for(i=0; i < BSIZE; i++)
		{
			m[peri_col_array_offset + idx] =  peri_col[i * BSIZE + idx];
