					...
					bool verbose)
{
	...
	mem = (char*)alignedMalloc(malloc_size);
	if(mem==NULL){
	...
	if(verbose){
		...
		printf("\nNumber of records = %ld, sizeof(record)=%zd, total=%zd\n",size,sizeof(record),size*sizeof(record));
		printf("Number of knodes = %ld, sizeof(knode)=%zd, total=%zd\n",nodeindex,sizeof(knode),(nodeindex)*sizeof(knode));
		printf("\nDone Transformation. Mem used: %ld\n", mem_used);
	}
	GetTime(two);
	time = TimeDiff(one, two) * 0.001;
...
print_leaves( node* root ) 
{
	...
	while (true) {
		...
		if (verbose_output) {
                  printf("%lx ", (unsigned long int)c->pointers[order - 1]);
                }
...
void sanity_check() {
  assert(DEFAULT_ORDER == DEFAULT_ORDER_2);
	...
	while (sscanf(commandPointer, "%c", &instruction) != EOF) {
	  ...
	  switch (instruction) {
			...
			case 'k':
			{
				...
				cl_long *currKnode;
				currKnode = (cl_long *)alignedMalloc(count*sizeof(cl_long));
				// INPUT: offset CPU initialization
				memset(currKnode, 0, count*sizeof(cl_long));

				// INPUT: offset CPU allocation
				cl_long *offset;
				offset = (cl_long *)alignedMalloc(count*sizeof(cl_long));
				// INPUT: offset CPU initialization
				memset(offset, 0, count*sizeof(cl_long));

				// INPUT: keys CPU allocation
				int *keys;
				keys = (int *)alignedMalloc(count*sizeof(int));
				...
				record *ans = (record *)alignedMalloc(sizeof(record)*count);
			...
			case 'j':
			{
				...
				cl_long *lastKnode;
				lastKnode = (cl_long *)alignedMalloc(count*sizeof(cl_long));
				// INPUT: offset CPU initialization
				memset (lastKnode, 0, count*sizeof(cl_long));

				// INPUT: offset_2 CPU allocation
				cl_long *offset_2;
				offset_2 = (cl_long *)alignedMalloc(count*sizeof(cl_long));
				// INPUT: offset CPU initialization
				memset (offset_2, 0, count*sizeof(cl_long));

				// INPUT: start, end CPU allocation
				int *start;
				start = (int *)alignedMalloc(count*sizeof(int));
				int *end;
				end = (int *)alignedMalloc(count*sizeof(int));
				...
				int *recstart;
				recstart = (int *)alignedMalloc(count*sizeof(int));
				int *reclength;
				reclength = (int *)alignedMalloc(count*sizeof(int));
				// OUTPUT: ans CPU initialization
				for(i = 0; i < count; i++){
					recstart[i] = 0;
					reclength[i] = 0;
...
static cl_context get_context() {
  ...
  cl_context context = clCreateContextFromType( ctxprop, device_type, NULL, NULL, &error );
  if( !context )
  {
