...
__kernel void compute_flux_contributions(
    int nelr,
    __global float* variables,
    __global float* fc_momentum_x,
    __global float* fc_momentum_y,
    __global float* fc_momentum_z,
    __global float* fc_density_energy) {
  for (int i = 0; i < nelr; ++i) {
...
__kernel void compute_flux(
    ...
    __global float* flux_contribution_momentum_x,
    __global float* flux_contribution_momentum_y,
    __global float* flux_contribution_momentum_z,
    __global float* flux_contribution_density_energy,
    int nelr){
  ...
  for (int i = 0; i < nelr; ++i) {
    ...
    FLOAT3_ASSIGN(flux_contribution_i_momentum_x,
                  flux_contribution_momentum_x[i + 0*nelr],
                  flux_contribution_momentum_x[i + 1*nelr],
                  flux_contribution_momentum_x[i + 2*nelr]);

    FLOAT3_ASSIGN(flux_contribution_i_momentum_y,
                  flux_contribution_momentum_y[i + 0*nelr],
                  flux_contribution_momentum_y[i + 1*nelr],
                  flux_contribution_momentum_y[i + 2*nelr]);

    FLOAT3_ASSIGN(flux_contribution_i_momentum_z,
                  flux_contribution_momentum_z[i + 0*nelr],
                  flux_contribution_momentum_z[i + 1*nelr],
                  flux_contribution_momentum_z[i + 2*nelr]);

    FLOAT3_ASSIGN(flux_contribution_i_density_energy,
                  flux_contribution_density_energy[i + 0*nelr],
                  flux_contribution_density_energy[i + 1*nelr],
                  flux_contribution_density_energy[i + 2*nelr]);
  
	
    float flux_i_density = (float)(0.0f);
    FLOAT3 flux_i_momentum;
    FLOAT3_ASSIGN(flux_i_momentum, 0.0f, 0.0f, 0.0f);
    ...
    for(j = 0; j < NNB; j++)
    {
      ...
      if(nb >= 0) 	// a legitimate neighbor
      {
        ...
        FLOAT3_ASSIGN(flux_contribution_nb_momentum_x,
                      flux_contribution_momentum_x[nb + 0*nelr],
                      flux_contribution_momentum_x[nb + 1*nelr],
                      flux_contribution_momentum_x[nb + 2*nelr]);
        FLOAT3_ASSIGN(flux_contribution_nb_momentum_y,
                      flux_contribution_momentum_y[nb + 0*nelr],
                      flux_contribution_momentum_y[nb + 1*nelr],
                      flux_contribution_momentum_y[nb + 2*nelr]);
        FLOAT3_ASSIGN(flux_contribution_nb_momentum_z,
                      flux_contribution_momentum_z[nb + 0*nelr],
                      flux_contribution_momentum_z[nb + 1*nelr],
                      flux_contribution_momentum_z[nb + 2*nelr]);
        FLOAT3_ASSIGN(flux_contribution_nb_density_energy,
                      flux_contribution_density_energy[nb + 0*nelr],
                      flux_contribution_density_energy[nb + 1*nelr],
                      flux_contribution_density_energy[nb + 2*nelr]);
