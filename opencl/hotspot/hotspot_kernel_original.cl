...
__kernel void hotspot(  int iteration,  //number of iteration
  ...
  int small_block_rows = BSIZE-iteration*2;//EXPAND_RATE
  int small_block_cols = BSIZE-iteration*2;//EXPAND_RATE
  ...
  for (int i=0; i<iteration ; i++){ 
    computed = false;
    if( IN_RANGE(tx, i+1, BSIZE-i-2) &&  \
        IN_RANGE(ty, i+1, BSIZE-i-2) &&  \
