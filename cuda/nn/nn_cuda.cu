...
TimeStamp tt[2];
double t_write, t_kernel, t_read;
int flag = 0;
double power = 0;
double energy = 0;
...
int main(int argc, char* argv[])
{
    ...
    */
...

    {
      ...
      // Write Buffer
      printf("%0.6lf [%.2fMB]\t",
	     t_write, (float)((sizeof(LatLong) * numRecords)/1e6));

      // Kernel
      printf("%0.6lf\t", t_kernel);

      // Read Buffer
      printf("%0.6lf [%.2fMB]\t",
	     t_read, (float)((sizeof(float) * numRecords)/1e6));
        
      printf("%0.6lf\n\n", t_write + t_kernel + t_read);
    }
    ...
    if (power != -1) // -1 --> failed to read energy values
    {
        ...
        printf("Power efficiency is %0.6lf 1/jouls.\n", 1.0/energy);
    }
